  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Бібліотека контенту</h1>
          <p className="text-slate-400 mt-1">Керування збереженими майстер-класами, записами та PDF-матеріалами.</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-4">
          <h2 className="text-lg font-bold text-white">Усі матеріали</h2>
          {content.length === 0 ? (
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-8 text-center text-slate-400">
              Контент не знайдено. Додайте перший майстер-клас або PDF!
            </div>
          ) : (
            content.map((item) => (
              <div key={item.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-start gap-4">
                ...
                <div>
                  <span className={`text-xs px-2 py-1 rounded-full font-medium ${item.isActive ? "bg-yellow-100 text-yellow-700" : "bg-slate-800 text-slate-300"}`}>
                    {item.isActive ? "Активно" : "Приховано"}
                  </span>
                </div>
              </div>
            ))
        ...
        <div>
          <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 shadow-sm sticky top-6">
            <h2 className="text-lg font-bold text-white mb-4">Додати контент</h2>
            <AddContentForm />
          </div>
        </div>
      </div>
